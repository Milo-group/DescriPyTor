...
def fit_and_evaluate_single_combination_regression(model, combination, r2_threshold=0.7):
    ...
    # Check if adjusted R² is above the threshold (falls back to R² if missing)
    t3=time.time()
    primary_r2 = evaluation_results.get('adj_r2', evaluation_results.get('r2', float('-inf')))
    if primary_r2 > r2_threshold:
