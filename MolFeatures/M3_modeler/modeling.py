...
def fit_and_evaluate_single_combination_regression(model, combination, r2_threshold=0.7):
    ...
    # ── Fit both estimators ───────────────────────────────────────────────────
    # model.fit()       → custom ridge (stores self.theta, used for prediction
    #                     intervals, coefficients, covariance matrix)
    # model.model.fit() → sklearn LinearRegression (required by model.evaluate()
    #                     and calculate_q2_and_mae via StandardScaler pipeline)
    t0 = time.time()
    model.fit(X, y)
    model.model.fit(X, y)
    ...
    # Removed r2_threshold gate: gating caused training R² to be mistaken
    # for Q² in reports, and silently skipped Q² for borderline combos.
    t3 = time.time()
    ...
    if 'scores' in result:
        try:
            ...
            adj_r2 = result['scores'].get('adj_r2', float('-inf'))
